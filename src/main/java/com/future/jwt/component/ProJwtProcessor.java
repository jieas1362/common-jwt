...
import static com.future.base.constant.common.ResponseElement.UNAUTHORIZED;
import static com.future.jwt.constant.JwtConfSchema.*;
import static com.future.jwt.constant.JwtDefaultElement.*;
...
public final class ProJwtProcessor<T> implements JwtProcessor<T> {
     ...
     * @param jwtConf
     */
    public ProJwtProcessor(JwtConf<T> jwtConf) {
        assertConf(jwtConf);
        ...
        this.gammaSecretArr = jwtConf.getGammaSecrets().stream().toArray(String[]::new);
        ...
        this.issuer = ofNullable(jwtConf.getIssuer()).filter(ProChecker::isNotBlank).orElse(ISSUER.identity);
        this.subject = ofNullable(jwtConf.getSubject()).filter(ProChecker::isNotBlank).orElse(SUBJECT.identity);
        this.audience = ofNullable(jwtConf.getAudience()).filter(ProChecker::isNotBlank).orElse(AUDIENCE.identity);
    }
